...
int main()
{
  //судоку масив, смятане на размера на масива
  // Грешно решение
  ...
  cout << "The size of the array is: " << array_size << endl; //принтираме размера на масива
  ...
  for (int i = 0; i < array_size; i++)
  {
    ...
    Двата резултата се присъждат като индекси на елемента от двумерния масив.
    array[i] се задава като стойност на текущия sudokuTable[][] елемент. */
  }
  // Тримерен масив за числата във судоку клетка
  ...
  for (int i = 0; i < 9; i++) // 
  {
    for (int j = 0; j < 9; j++)
    {
      ...
      // Израз за присъждане на числата от двумерния масив (таблично) в тримерен - по клетки (3х3)
  ...
  for (int d = 1; d < 10; d++) //цикъл за сравняване на цифрите от 1 до 9
  {
    for (int i = 0; i < 9; i++)
    {
      ...
      // Проверка дали броячите не показват повече от 2 повторения
      if (count_r > 1){
        cout << "The number "<< d << " appears " << count_r << " times in row " << i+1 << endl;
        isValid = 0; //повече от 2? Судоку решението не е валидно
      }  
      // Проверка дали всички числа от 1 до 9 са въведени само веднъж във всяка колона,  ако не извеждаме съобщение
      if (count_c > 1){
        cout << "The number "<< d << " appears " << count_c << " times in column " << i+1 << endl;
        isValid = 0; // повече от 2? Судоку решението не е валидно
      }
      count_r = 0; // ресетваме броячите за следващата проверка
      count_c = 0;
    }
  }
  ...
  for (int d = 1; d < 10; d++){
    for (int i=0; i<3; i++){
        for (int j=0; j<3; j++){
            ...
            if (count_c > 1){ // използваме съчия брояч count_c който е ресетнат на 0 след последния цикъл по-горе
